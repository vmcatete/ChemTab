...
public class TerrainToolkitEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		switch (terrain.toolModeInt) {
			...
			// -------------------------------------------------------------------------------------------------------- TEXTURING TOOLS
			case 2:
			Terrain terA = (Terrain) terrain.GetComponent(typeof(Terrain));
			if (terA == null) {
				return;
			}
			TerrainData terAData = terA.terrainData;
			terrain.splatPrototypes = terAData.splatPrototypes;
			EditorGUILayout.Separator();
			float mouseX;
			EditorGUILayout.BeginHorizontal();
			GUI.skin = terrain.guiSkin;
			...
			EditorGUILayout.EndHorizontal();
			if (GUI.changed) {
				terAData.splatPrototypes = terrain.splatPrototypes;
			}
